...
MockModule = {
  ...
  */
  ...
  createMockEntity:function( layer, dataIndex){
    ...
    var mock = {type:"Tile",head:"2D," + this._renderMethod + ",Tile,Tile" + layer.data[dataIndex] + "," + layer.name};
  ...
  createMockEmpty:function( layer, dataIndex){
    var column = dataIndex % layer.width;
    var row = Math.floor((dataIndex / layer.width));
    var mock = {type:"Empty", head:"2D," + this._renderMethod + ",TileEmpty," + layer.name};
    this.setPosition( column, row, mock );
    return mock;
  },
